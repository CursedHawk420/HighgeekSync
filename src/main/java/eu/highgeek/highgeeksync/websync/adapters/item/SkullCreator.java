...
public class SkullCreator {
    ...
    public static ItemStack itemWithBase64AndNameAndUUID(ItemStack item, String base64, String name, String uuid, @Nullable String signature) {
        ...
        SkullMeta meta = (SkullMeta) item.getItemMeta();
        mutateItemMetaWithNameAndUUID(meta, base64, name, uuid, signature);
    ...
    private static GameProfile makeProfileWithNameAndUUUID(String b64, String name, String uuid, @Nullable String signature) {
        // random uuid based on the b64 string
...
//        );
        ...
        if (signature != null){
            profile.getProperties().put("textures", new Property("textures", b64, signature));
        }
        else {
            profile.getProperties().put("textures", new Property("textures", b64));
        }
    ...
    public static void mutateItemMetaWithNameAndUUID(SkullMeta meta, String b64, String name, String uuid, @Nullable String signature) {
        try {
            if (metaSetProfileMethod == null) {
                metaSetProfileMethod = meta.getClass().getDeclaredMethod("setProfile", GameProfile.class);
                metaSetProfileMethod.setAccessible(true);
            }
            metaSetProfileMethod.invoke(meta, makeProfileWithNameAndUUUID(b64, name, uuid, signature));
        } catch (NoSuchMethodException | IllegalAccessException | InvocationTargetException ex) {
            // if in an older API where there is no setProfile method,
            // we set the profile field directly.
            try {
                ...
                metaProfileField.set(meta, makeProfileWithNameAndUUUID(b64, name, uuid, signature));
    ...
    private static void applySkullLore(SkullMeta headMeta, @Nullable String extra){
        ArrayList<String> lore = new ArrayList<>();
        lore.add(" ");
        lore.add(ChatColor.BLUE+""+ChatColor.ITALIC+"Player Heads");
